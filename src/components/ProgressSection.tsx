...
const Circle: React.FC<CircleProps> = ({
  ...
  const sizeClasses = size === 'large' ? 'w-20 h-20 text-sm' : 'w-12 h-12 text-xs';
  return <button onClick={onClick} className={`
        ${sizeClasses} 
        ${getStatusColors()}
        rounded-full border-3 flex items-center justify-center
        z-50 relative font-medium
...
const SurahNode: React.FC<{
  ...
  return <div className="relative mb-10" style={{
    height: '160px'
  }}>
      ...
      left: currentX - 40,
      // Center the surah circle
      top: currentY - 40,
      zIndex: 30
    }}>
        {/* Main surah circle */}
        <Circle status={surahStatus} size="large" onClick={() => onSurahSelect?.(surah.id)} isSpecial={isCurrentSurah} proficiency={getSurahProficiency?.(surah.id)}>
          <div className="text-center leading-tight">
            <div className="font-semibold font-arabic">{surah.arabicName}</div>
            {getSurahProficiency?.(surah.id) && <div className="text-xs mt-1">
  ...
  return <div className="w-80 h-full bg-gradient-to-t from-purple-50 via-blue-50 via-green-50 to-orange-50 relative overflow-hidden">
      ...
      <ScrollArea className="h-[calc(100vh-200px)] relative">
        <div className="relative pt-8 pb-32">
          ...
          {effectiveCompletedCount === totalSurahs && <div className="flex flex-col items-center mb-8">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-white via-white/95 to-transparent z-50">
        <div className="flex justify-center">
          <button onClick={() => onSurahSelect(114)} // Start with Surat Al-Nas
